...
class SegmentalKMeans (object):

    def __init__ (self, fileName, T, N):
    ...
    def get_segments (self):
        ...
        self.data = self.data[['temp', 'atemp', 'hum', 'windspeed']]
        self.data = self.data[:-1]

        self.w = self.data.shape[0] // self.T

        print('w, T = ', w, self.T)
        print('N = ', self.N)

        # Split the data into a wxT numpy matrix
        sequences = self.data.values
        sequences = sequences.reshape(self.w, self.T, self.data.shape[1])

        return sequences

    def cluster_k_means (sequences, N, T, w):
    ...
    def get_init_probability (sequences, data, states, N, T):
        # Calculate initial probabilities
        pi_vector = [0 for i in range(0, N)]

        observations_all, counts_total = np.unique(data.values, axis=0, return_counts=True)
